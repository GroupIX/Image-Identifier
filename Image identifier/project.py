...
#New lines I am adding
import tensorflow as tf
import numpy as np
import wikipedia as wiki

#Loading VGG16 model from TensorFlow library
...
def identify_image(image_path):
    ##All lines starting with image mod the image to make it acceptable o the model
    image = tf.keras.preprocessing.image.load_img(image_path, target_size=(IMAGE_SIZE, IMAGE_SIZE))
    image = tf.keras.preprocessing.image.img_to_array(image)
    image = np.expand_dims(image, axis=0)
    prediction_result = model.predict(image, batch_size=1)#Return model prediction 
    best_prediction = tf.keras.applications.imagenet_utils.decode_predictions(prediction_result, top=1)#Takes numerical values given above and labels for top predictions.
    print(wiki.summary(article, sentences= 10))
